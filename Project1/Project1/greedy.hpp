...
template<typename T>
bool contains2(std::deque<T> queue, T node) {
	for (auto item : queue) {
		if (item == node) {
			return true;
		}
...
size_t index_of2(Node node, const Node nodes[N]) {
...
template<typename T>
bool check2(priority_queue<T, std::vector<T>, std::greater<T>> frontier, int test) {
	...
	Cost(*heuristic)(Node current, Node goal, Cost **sw))
{
	...
	frontier.emplace(0, index_of2<Node, N>(START, vertex));
	...
	while (!frontier.empty()) {
		...
		for (size_t n = 0; n < N; n++) {
			...
			if (graph[current][n] == 0 || contains2(explored, n)) { continue; }
			...
			if (aggregateCost.find(n) == aggregateCost.end() || cost < aggregateCost[n]) {
				...
				if (!check<El>(frontier, n)) {
					frontier.emplace(heuristic(vertex[n], GOAL, sw), n);
				}
			}
		}
	}
